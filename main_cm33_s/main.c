...
*    1. Enables the PPCA configuration block (CNFG init + enable)
*    2. Boots PPCA Core 0 and PPCA Core 1 from their flash images
*    3. Remains in an idle loop
* Add application logic here or in the PPCA core stub files.
*
* Parameters:
*  void
*
...
int main(void)
{
    ...
    /* Initialize PPCA configuration block and enable the PPCA subsystem
       before booting the PPCA cores so the memory regions are accessible */
    Cy_PPCA_CNFG_Init(PPCA_CNFG_HW, &PPCA_CNFG_config);
    Cy_PPCA_Enable(PPCA_CNFG_HW);

    /* Boot PPCA Core 0 from its flash image */
    Cy_System_Init_CPU0((void*)CORE0_IMAGE_ADDRESS, PPCA0_IMAGE_SIZE);
    /* Boot PPCA Core 1 from its flash image */
    Cy_System_Init_CPU1((void*)CORE1_IMAGE_ADDRESS, PPCA1_IMAGE_SIZE);

    /* Main core idle loop – add application logic here */
    for (;;)
    {
